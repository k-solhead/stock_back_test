...
def daily_stock(db_file_name):
  ...
  cur = conn.cursor()
  cur.execute("DELETE FROM new_brands WHERE date >= ?", (d_today,))
  cur.execute("DELETE FROM delete_brands WHERE date >= ?", (d_today,))
  cur.execute("DELETE FROM divide_union_data WHERE date_of_right_allotment >= ?", (d_today,))
  conn.commit()
  conn.close()
  ...
  cur = conn.execute(sql1, (d_today,))    #　本日に上場した銘柄を取り出す
  ...
  cur = conn.execute(sql2, (d_today,))    #　本日に廃止になった銘柄を取り出す
